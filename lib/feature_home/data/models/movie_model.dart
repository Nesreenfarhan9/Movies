...
class MovieDetails {
  ...
  factory MovieDetails.fromJson(Map<String, dynamic> json) {
    return MovieDetails(
      ...
      summary: json['description_full'] ?? 'No summary available.', // Assigning summary
      cast: (json['cast'] as List<dynamic>?)?.map((actor) {
        return {
          'name': (actor['name'] ?? 'Unknown').toString(),
          'character': (actor['character'] ?? 'Unknown').toString(),
          'image': (actor['image'] ?? '').toString(),
        };
      }).toList() ?? [],
